...
public class GhostsP1 extends GhostController {
	private EnumMap<GHOST, MOVE> moves = new EnumMap<GHOST, MOVE>(GHOST.class);
	private MOVE[] allMoves = MOVE.values();
	private Random rnd = new Random();
	private int range = 50;
	private int pacmanMaxDistanceToPill = 40;
			...
			if (game.doesGhostRequireAction(ghostType)) {
				...
				if (dist > range) {
					...
					if (currentGhostDest[ghostType.ordinal()] != -1) {
						...
						MOVE move = game.getApproximateNextMoveTowardsTarget(ghostNode, currentGhostDest[ghostType.ordinal()], ghostLastMoveMade, DM.PATH);
					...
					// 2 van a por pacman evitándose, marcan en el array de marcas las intersecciones
					if (ghostType == GHOST.BLINKY || ghostType == GHOST.PINKY) {
						MOVE move = game.getApproximateNextMoveTowardsTarget(ghostNode, pacmanNode, ghostLastMoveMade,
								DM.PATH);
					...
					// elige un nodo aleatorio del mapa (que sea interseccion) y mira si la distancia hasta pacman es menos
					// que 2 veces el rango
						...
						System.out.println("Nodo random " + ghostType + ": " + randomNode);

						currentGhostDest[ghostType.ordinal()] = randomNode;

						MOVE move = game.getApproximateNextMoveTowardsTarget(ghostNode, randomNode, ghostLastMoveMade,
								DM.PATH);
				...
				// pacman en rango
					...
					if (pacmanDistToPP < pacmanMaxDistanceToPill) {
						int ghostDistanceToPP = game.getShortestPathDistance(ghostNode, closestPP, pacmanLastMoveMade);
